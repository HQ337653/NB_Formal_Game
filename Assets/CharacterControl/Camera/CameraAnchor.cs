...
namespace NBGame.CameraMovement
{
    public class CameraAnchor : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            if (Follow != null)
            {
                transform.position = Vector3.SmoothDamp(transform.position, new Vector3(Follow.transform.position.x, Follow.transform.position.y + shiftY, Z), ref velocity, smoothTime);
            }
        }
